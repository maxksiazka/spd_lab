...
def generate_pm_cmax_benchmark(n_jobs, n_machines, min_p=10, max_p=100, folder="benchmarks"):
    """
    Generates a single Pm||Cmax benchmark instance.
    """
    os.makedirs(folder, exist_ok=True)
    filename = f"pm_{n_jobs}_{n_machines}.txt"
    ...
    with open(filepath, 'w') as f:
        ...
        for p in processing_times:
            f.write(f"{p}\n")
    with open("benchmarks/README.txt",'a') as f:
...
if __name__ == "__main__":
    ...
    for m, n_list in test_configs.items():
        for n in n_list:
            generate_pm_cmax_benchmark(n_jobs=n, n_machines=m)
    print("\nDone! Benchmarks saved to the 'benchmarks/' directory.")
